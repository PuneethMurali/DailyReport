...
namespace Day14
{
    ...
    public class SavingsAccount : IBank
    {
        ...
        public bool WithDrawAmount(double amount)
        {
            if (balance < amount)
            {
                Console.WriteLine("have amount  less than  amout ");
                return false;
            }
            else if (TodaysWithDrawAmount + amount > perdayWithDrawLimit)
            {
                Console.WriteLine("withdraw attempt failed becuae per day we can withdraw only 10000");
                return false;
            }
            else
            {
                balance = balance - amount;
                TodaysWithDrawAmount = TodaysWithDrawAmount + amount;
                Console.WriteLine(" Balnce  in acc is " + balance);
                return true;
            }
        }
    }
    ...
    class Testbank
    {
        ...
        {
            IBank b1 = new SavingsAccount();
            b1.DepositAmount(20000);
            b1.WithDrawAmount(5000);
            double bal = b1.CheckBal();
            Console.WriteLine(" after deposit and withdraw " + bal);
            IBank b2 = new CurrentAccount();
            b2.DepositAmount(4500);
            b2.WithDrawAmount(3000);
            double a = b2.CheckBal();
        }
    }
}
